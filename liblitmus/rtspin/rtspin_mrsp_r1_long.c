...
static int job(double wcet, double bcet, double acet, double program_end, int chunk_lock_od)
{
	double exec_cost;
	double critical_section = 0;
	double chunk_1 = 0;
	double chunk_2 = 0;
	...
	if (wctime() > program_end)
		...
		if (chunk_lock_od >= 0){
			if (exec_cost < 0.0003){
				critical_section = 0.8*exec_cost;
				chunk_1 = chunk_2 = 0.1*exec_cost;
			} else {
			/* get the random critical section */
				critical_section = 0.3*0.001*drand48();
				chunk_1 = drand48()*(exec_cost-critical_section);
				chunk_2 = exec_cost - critical_section - chunk_1;
			}

			loop_for(chunk_1, program_end + 1);
			litmus_lock(chunk_lock_od);
			loop_for(critical_section, program_end + 1);
			litmus_unlock(chunk_lock_od);
			loop_for(chunk_2, program_end + 2);	
		} else {
			
			loop_for(exec_cost, program_end + 1);
		}
...
#define OPTSTR "p:c:wlveo:F:s:m:M:L:A:q:r:X:Q:iRu:Bhd:C:"
int main(int argc, char** argv)
{
	...
	if (protocol >= 0) {
		chunk_lock_od = litmus_open_lock(protocol, chunk_id, lock_namespace, &prio_per_cpu[4]);
        }
