...
static int job(double wcet, double bcet, double acet, double program_end, int chunk_lock_od[], int task_num)
{
	...
	/* get the whole execution time from normal distribution */
		...
		if (exec_cost < 0.0009){
			chunk_length[1] = drand48()*0.4*exec_cost;	
			chunk_length[3] = drand48()*0.3*exec_cost;
			chunk_length[5] = drand48()*0.3*exec_cost;
		} else {
			chunk_length[1] = drand48()*0.0003;
			chunk_length[3] = drand48()*0.0003;
			chunk_length[5] = drand48()*0.0003;
		}
