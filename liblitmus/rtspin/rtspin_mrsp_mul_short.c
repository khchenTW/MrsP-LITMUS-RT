...
const char *usage_msg =
	...
	"    -M migrate_CPU    migrate the task to\n"
        "    -L min time       minimum execution time of the task\n"
        "    -A avg time       average execution time of the task\n"
...
double gaussrand(double min, double avg, double max)
{
...
static int job(double wcet, double bcet, double acet, double program_end, int chunk_lock_od[], int task_num)
{
	double exec_cost;
	double chunk_length[7] = {0};
	int i = 0;
	int chunk_id[40][7]= {
		{0,1,0,3,0,4,0},//0
		{0,2,0,3,0,4,0},
	...
	/* get the whole execution time from normal distribution */
		exec_cost = gaussrand(bcet, acet, wcet);
		exec_cost = 0.001*exec_cost;
	
		if (exec_cost < 0.0003){
			chunk_length[1] = drand48()*0.2*exec_cost;	
			chunk_length[3] = drand48()*0.2*exec_cost;
			chunk_length[5] = drand48()*0.2*exec_cost;
			chunk_length[0] = drand48()*0.1*exec_cost;
			chunk_length[2] = drand48()*0.1*exec_cost;
			chunk_length[4] = drand48()*0.1*exec_cost;
			chunk_length[6] = exec_cost-chunk_length[0]-chunk_length[1]-chunk_length[2]-chunk_length[3]-chunk_length[4]-chunk_length[5];
		} else {
			chunk_length[1] = drand48()*0.0001;
			chunk_length[3] = drand48()*0.0001;
			chunk_length[5] = drand48()*0.0001;
			chunk_length[0] = (exec_cost-chunk_length[1]-chunk_length[3]-chunk_length[5])*0.4;
			chunk_length[2] = chunk_length[4] = chunk_length[6] = 0.5*chunk_length[0];
		}
		for (i=0;i<7;i++){
			if(chunk_id[task_num][i]==0){
				loop_for(chunk_length[i],program_end+i+1);
			} else {
				litmus_lock(chunk_lock_od[chunk_id[task_num][i]-1]);
				loop_for(chunk_length[i],program_end+i+1);
				litmus_unlock(chunk_lock_od[chunk_id[task_num][i]-1]);
			}
		}
		sleep_next_period();
		return 1;	
...
#define OPTSTR "p:c:wlveo:F:s:m:M:N:L:A:q:r:X:Q:iRu:Bhd:C:"
int main(int argc, char** argv)
{
	...
	//double cs_length = 1; /* millisecond */
        ...
        int chunk_id[6] = {1,2,3,4,5,6};
        //int prio_per_cpu[4] = {4,8,15,23};
        int chunk_lock_od[6] = {-1};
	int task_num = -1;
	...
	while ((opt = getopt(argc, argv, OPTSTR)) != -1) {
		...
		case 'p':
			...
			break;
		case 'L':
			min_exec_time = atof(optarg);
			break;
                case 'A':
                        avg_exec_time = atof(optarg);
                        break;
		case 'N':
	...
	if (protocol >= 0) {
		chunk_lock_od[0] = litmus_open_lock(protocol, chunk_id[0], lock_namespace, init_prio_per_cpu(4,4,12,19,28));
		chunk_lock_od[1] = litmus_open_lock(protocol, chunk_id[1], lock_namespace, init_prio_per_cpu(4,5,13,19,28));
		chunk_lock_od[2] = litmus_open_lock(protocol, chunk_id[2], lock_namespace, init_prio_per_cpu(4,4,11,20,29));
		chunk_lock_od[3] = litmus_open_lock(protocol, chunk_id[3], lock_namespace, init_prio_per_cpu(4,4,11,19,30));
		chunk_lock_od[4] = litmus_open_lock(protocol, chunk_id[4], lock_namespace, init_prio_per_cpu(4,6,12,20,31));
		chunk_lock_od[5] = litmus_open_lock(protocol, chunk_id[5], lock_namespace, init_prio_per_cpu(4,6,11,23,28));
	...
	if (file) {
		...
		for (cur_job = 0; cur_job < num_jobs; ++cur_job) {
			...
			job(wcet_ms, min_exec_time, avg_exec_time, start + duration,
                           chunk_lock_od, task_num);
		}
	} else {
		double acet;
		do {
		...
		} while (job(wcet_ms, min_exec_time, avg_exec_time, start + duration,
			   chunk_lock_od, task_num));
	}
